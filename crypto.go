...
// Address generates new Vrp address from the `publicKey` and `chainID`. The function returns an empty string in case of error.
func (c *crypto) Address(publicKey PublicKey, chainID VrpChainID) Address {
...
// AddressFromSeed returns a new Vrp address produced from the `seed` and `chainID`.
func (c *crypto) AddressFromSeed(seed Seed, chainID VrpChainID) Address {
...
// VerifyAddress returns true if `address` is a valid Vrp address for the given `chainID`.
func (c *crypto) VerifyAddress(address Address, chainID VrpChainID) bool {
	ab := c.Base58Decode(string(address))
	if len(ab) != addressSize {
		return false
	}
